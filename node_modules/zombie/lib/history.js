...
var Location = (function () {
  ...
  _createClass(Location, [{
    ...
    set: function (value) {
      var url = _core.Object.assign(URL.parse(this._url), { hash: value });
    ...
    set: function (value) {
      var url = _core.Object.assign(URL.parse(this._url), { host: value });
      this.assign(URL.format(url));
    }
  }, {
    key: 'pathname',
    ...
    set: function (value) {
      var url = _core.Object.assign(URL.parse(this._url), { pathname: value });
      this.assign(URL.format(url));
    }
  }, {
    key: 'port',
    ...
    set: function (value) {
      var url = _core.Object.assign(URL.parse(this._url), { port: value });
      this.assign(URL.format(url));
    }
  }, {
    key: 'protocol',
    ...
    set: function (value) {
      var url = _core.Object.assign(URL.parse(this._url), { protocol: value });
      this.assign(URL.format(url));
    }
  }, {
    key: 'search',
    ...
    set: function (value) {
      var url = _core.Object.assign(URL.parse(this._url), { search: value });
      this.assign(URL.format(url));
    }
  }]);
...
function hashChange(entry, url) {
  ...
  var aBase = _url$split[0];
  var aHash = _url$split[1];

  var _entry$url$split = entry.url.split('#');

  var bBase = _entry$url$split[0];
  var bHash = _entry$url$split[1];
...
var Entry = (function () {
  ...
  Entry.prototype.destroy = function destroy(keepAlive) {
    if (this.next) {
  ...
  Entry.prototype.append = function append(newEntry, keepAlive) {
...
var History = (function () {
  ...
  History.prototype.open = function open(args) {
  ...
  History.prototype.addEntry = function addEntry(window) {
    var _this = this;

    var url = arguments[1] === undefined ? window.location.href : arguments[1];
    var pushState = arguments[2] === undefined ? undefined : arguments[2];
    return (function () {
      var entry = new Entry(window, url, pushState);
      if (_this.current) {
        _this.current.append(entry);
        _this.current = entry;
      } else {
        _this.first = entry;
        _this.current = entry;
  ...
  History.prototype.replaceEntry = function replaceEntry(window) {
    var _this2 = this;
    ...
    return (function () {
      var entry = new Entry(window, url, pushState);
      if (_this2.current === _this2.first) {
        if (_this2.current) _this2.current.destroy(window);
        _this2.first = entry;
        _this2.current = entry;
      } else {
        _this2.current.prev.append(entry, window);
        _this2.current = entry;
      }
      _this2.focus(window);
    })();
  ...
  History.prototype.updateLocation = function updateLocation(window, url) {
  ...
  History.prototype.submit = function submit(args) {
    args.browser = this.browser;
    args.history = this;
  ...
  History.prototype.assign = function assign(url) {
    var _this3 = this;
    ...
    if (hashChange(this.current, url)) {
      (function () {
        ...
        _this3.updateLocation(window, url);
        _this3.addEntry(window, url); // Reuse window with new URL
  ...
  History.prototype.replace = function replace(url) {
    ...
    if (hashChange(this.current, url)) {
      (function () {
        var window = _this4.current.window;

        _this4.replaceEntry(window, url); // Reuse window with new URL
        var event = window.document.createEvent('HTMLEvents');
        event.initEvent('hashchange', true, false);
        window._eventQueue.enqueue(function () {
          window.dispatchEvent(event);
  ...
  History.prototype.go = function go(amount) {
    var _this5 = this;
    ...
    if (this.current && was && this.current !== was) {
      (function () {
        var window = _this5.current.window;

        _this5.updateLocation(window, _this5.current.url);
        _this5.focus(window);

        if (_this5.current.pushState || was.pushState) {
          // Created with pushState/replaceState, send popstate event if navigating
          // within same host.
          var oldHost = URL.parse(was.url).host;
          var newHost = URL.parse(_this5.current.url).host;
          if (oldHost === newHost) {
            (function () {
              var popstate = window.document.createEvent('HTMLEvents');
              popstate.initEvent('popstate', false, false);
              popstate.state = _this5.current.pushState;
              window._eventQueue.enqueue(function () {
                window.dispatchEvent(popstate);
              });
            })();
          }
        } else if (hashChange(was, _this5.current.url)) {
          (function () {
  ...
  History.prototype.pushState = function pushState(state, title) {
    var url = arguments[2] === undefined ? this.url : arguments[2];
  ...
  History.prototype.replaceState = function replaceState(state, title) {
    var url = arguments[2] === undefined ? this.url : arguments[2];
  ...
  History.prototype.dump = function dump() {
