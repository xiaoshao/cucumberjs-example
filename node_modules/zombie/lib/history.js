...
var History = (function () {
  ...
  History.prototype.replaceEntry = function replaceEntry(window) {
    ...
    return (function () {
      var entry = new Entry(window, url, pushState);
      if (_this.current === _this.first) {
        if (_this.current) _this.current.destroy(window);
        _this.first = entry;
        _this.current = entry;
      } else {
        _this.current.prev.append(entry, window);
        _this.current = entry;
      }
      _this.focus(window);
    })();
  ...
  History.prototype.assign = function assign(url) {
    ...
    if (hashChange(this.current, url)) {
      ...
      this.addEntry(_document.parentWindow, url);
    }
  };
  ...
  History.prototype.replace = function replace(url) {
    ...
    if (hashChange(this.current, url)) {
      (function () {
        ...
        _this.replaceEntry(window, url); // Reuse window with new URL
        var event = window.document.createEvent("HTMLEvents");
        event.initEvent("hashchange", true, false);
        window._eventQueue.enqueue(function () {
          window.dispatchEvent(event);
      ...
      var _document = loadDocument(args);
      this.replaceEntry(_document.parentWindow, url);
    }
  };

  History.prototype.reload = function reload() {
    var window = this.current.window.window;
  ...
  History.prototype.go = function go(amount) {
    ...
    if (this.current && was && this.current !== was) {
      (function () {
        ...
        _this.updateLocation(window, _this.current.url);
        _this.focus(window);

        if (_this.current.pushState || was.pushState) {
          // Created with pushState/replaceState, send popstate event if navigating
          // within same host.
          var oldHost = URL.parse(was.url).host;
          var newHost = URL.parse(_this.current.url).host;
          if (oldHost === newHost) {
            (function () {
              var popstate = window.document.createEvent("HTMLEvents");
              popstate.initEvent("popstate", false, false);
              popstate.state = _this.current.pushState;
              window._eventQueue.enqueue(function () {
                window.dispatchEvent(popstate);
              });
            })();
          }
        } else if (hashChange(was, _this.current.url)) {
          (function () {
