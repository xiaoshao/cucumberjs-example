...
var Browser = (function (_EventEmitter) {
  function Browser() {
    ...
    this.on('console', function (level, message) {
      if (_this3.silent) _this3._debug('>> ' + message);else console.log(message);
    }).on('log', function () {
      ...
      _this3._debug(format.apply(undefined, args));
    });
    ...
    this.on('request', function (request) {
      ...
      _this3._debug('%s %s => %s', request.method, response.url, response.status);
    }).on('redirect', function (request, response) {
      _this3._debug('%s %s => %s %s', request.method, request.url, response.status, response.headers.get('Location'));
    }).on('loaded', function (document) {
      _this3._debug('Loaded document %s', document.location.href);
    }).on('xhr', function (eventName, url) {
      _this3._debug('XHR %s %s', eventName, url);
    });

    // Logging windows/tabs/navigation
    this.on('opened', function (window) {
      _this3._debug('Opened window %s %s', window.location.href, window.name || '');
    }).on('closed', function (window) {
      _this3._debug('Closed window %s %s', window.location.href, window.name || '');
    });
    ...
    this.on('link', function (url) {
      _this3._debug('Follow link to %s', url);
    }).on('submit', function (url) {
      _this3._debug('Submit form to %s', url);
    });

    // Logging event loop
    this._eventLoop.on('setTimeout', function (fn, delay) {
      _this3._debug('Fired setTimeout after %dms delay', delay);
      _this3.emit('setTimeout', fn, delay);
    }).on('setInterval', function (fn, interval) {
      _this3._debug('Fired setInterval every %dms', interval);
      _this3.emit('setInterval', fn, interval);
    }).on('serverEvent', function () {
      _this3._debug('Server initiated event');
      _this3.emit('serverEvent');
    }).on('idle', function (timedOut) {
      if (timedOut) _this3._debug('Event loop timed out');else _this3._debug('Event loop is empty');
      _this3.emit('idle');
    ...
    this.on('error', function (error) {
      ...
      _this3._debug(error.stack);
    });

    // Sets the browser options.
    options = options || {};
    for (var _iterator = BROWSER_OPTIONS, _isArray = _core.Array.isArray(_iterator), _i = 0, _iterator = _isArray ? _iterator : _core.getIterator(_iterator);;) {
      var _ref;
    ...
    for (var _iterator2 = Browser._extensions, _isArray2 = _core.Array.isArray(_iterator2), _i2 = 0, _iterator2 = _isArray2 ? _iterator2 : _core.getIterator(_iterator2);;) {
      var _ref2;
  ...
  Browser.prototype.open = function open() {
    var _ref14 = arguments[0] === undefined ? {} : arguments[0];

    var url = _ref14.url;
    var name = _ref14.name;
    var referrer = _ref14.referrer;
  ...
  Browser.prototype.wait = function wait(options, callback) {
    ...
    if (callback) _eventLoop.wait(waitDuration, completionFunction, callback);else {
  ...
  Browser.prototype.waitForServer = function waitForServer(options, callback) {
    ...
    if (callback) {
      this._eventLoop.once('serverEvent', function () {
        _this4.wait(options, callback);
      });
      return null;
    }

    return new Bluebird(function (resolve) {
      _this4._eventLoop.once('serverEvent', function () {
        resolve(_this4.wait(options, null));
      });
    });
  };
  ...
  Browser.prototype._wait = function _wait(options, callback) {
    ...
    var lazyResolve = function lazyResolve() {
      if (!promise) promise = _this5.wait(options, null);
      return promise;
    };
  ...
  Browser.prototype.queryAll = function queryAll() {
    ...
    if (_core.Array.isArray(selector)) {
      return selector;
    }if (selector instanceof DOM.Element) {
      return [selector];
    }if (selector) {
      var elements = context.querySelectorAll(selector);
      return _core.Array.from(elements);
    } else {
      return [];
    }
  };
  ...
  Browser.prototype.link = function link(selector) {
    ...
    for (var _iterator3 = _core.Array.from(this.querySelectorAll('body a')), _isArray3 = _core.Array.isArray(_iterator3), _i3 = 0, _iterator3 = _isArray3 ? _iterator3 : _core.getIterator(_iterator3);;) {
      var _ref3;
  ...
  Browser.prototype.field = function field(selector) {
    ...
    for (var _iterator4 = this.queryAll('input[name],textarea[name],select[name]'), _isArray4 = _core.Array.isArray(_iterator4), _i4 = 0, _iterator4 = _isArray4 ? _iterator4 : _core.getIterator(_iterator4);;) {
      var _ref4;
    ...
    for (var _iterator5 = this.queryAll('label'), _isArray5 = _core.Array.isArray(_iterator5), _i5 = 0, _iterator5 = _isArray5 ? _iterator5 : _core.getIterator(_iterator5);;) {
      var _ref5;
  ...
  Browser.prototype._findOption = function _findOption(selector, value) {
    ...
    var options = _core.Array.from(field.options);
    for (var _iterator6 = options, _isArray6 = _core.Array.isArray(_iterator6), _i6 = 0, _iterator6 = _isArray6 ? _iterator6 : _core.getIterator(_iterator6);;) {
      var _ref6;
    ...
    for (var _iterator7 = options, _isArray7 = _core.Array.isArray(_iterator7), _i7 = 0, _iterator7 = _isArray7 ? _iterator7 : _core.getIterator(_iterator7);;) {
      var _ref7;
    ...
    for (var _iterator8 = options, _isArray8 = _core.Array.isArray(_iterator8), _i8 = 0, _iterator8 = _isArray8 ? _iterator8 : _core.getIterator(_iterator8);;) {
      var _ref8;
  ...
  Browser.prototype.button = function button(selector) {
    ...
    for (var _iterator9 = _core.Array.from(this.querySelectorAll('button')), _isArray9 = _core.Array.isArray(_iterator9), _i9 = 0, _iterator9 = _isArray9 ? _iterator9 : _core.getIterator(_iterator9);;) {
      var _ref9;
    ...
    var inputs = _core.Array.from(this.querySelectorAll('input[type=submit],button'));
    for (var _iterator10 = inputs, _isArray10 = _core.Array.isArray(_iterator10), _i10 = 0, _iterator10 = _isArray10 ? _iterator10 : _core.getIterator(_iterator10);;) {
      var _ref10;
    ...
    for (var _iterator11 = inputs, _isArray11 = _core.Array.isArray(_iterator11), _i11 = 0, _iterator11 = _isArray11 ? _iterator11 : _core.getIterator(_iterator11);;) {
      var _ref11;
  ...
  Browser.prototype.saveCookies = function saveCookies() {
    ...
    for (var _iterator12 = this.cookies.sort(Tough.cookieCompare), _isArray12 = _core.Array.isArray(_iterator12), _i12 = 0, _iterator12 = _isArray12 ? _iterator12 : _core.getIterator(_iterator12);;) {
      var _ref12;
  ...
  Browser.prototype.loadCookies = function loadCookies(serialized) {
    for (var _iterator13 = serialized.split(/\n+/), _isArray13 = _core.Array.isArray(_iterator13), _i13 = 0, _iterator13 = _isArray13 ? _iterator13 : _core.getIterator(_iterator13);;) {
      var _ref13;
  ...
  Browser.prototype.fetch = function fetch(input, init) {
