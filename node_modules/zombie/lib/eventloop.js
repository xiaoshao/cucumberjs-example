...
module.exports = (function (_EventEmitter) {
  ...
  EventLoop.prototype.wait = function wait(waitDuration, completionFunction, callback) {
    ...
    // further
    var timer = _core.global.setTimeout(function(){console.log("<><><><><>");timeout();}, waitDuration);
    var timeoutOn = Date.now() + waitDuration;
    // console.log(timer);
    console.log(timeoutOn);
    console.log("###############################");
    ...
    function ontick(next) {
      ...
      if (next >= timeoutOn) {
        console.log("+++++++++++++++++");
        console.log("next: " + next + " timeoutOn " + timeoutOn);
        timeout();
        return;
      }
    ...
    eventLoop.once("idle", function(){console.log("idle"); done()});

    // Stop on first error reported (document load, script, etc)
    // Event loop errors also propagated to the browser
    eventLoop.browser.once("error", function(){console.log("error");done();});
  };
    ...
    setImmediate(function () {
      ...
      try {
        ...
        if (_event) {
          ...
          _this.emit("tick", 0);else if (jsdomQueue.tail) {
          jsdomQueue.resume();
          _this.run();
        } else {
