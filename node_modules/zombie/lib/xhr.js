...
var XMLHttpRequest = (function (_DOM$EventTarget) {
  ...
  _inherits(XMLHttpRequest, _DOM$EventTarget);
  ...
  XMLHttpRequest.prototype.open = function open(method, url, useAsync, user, password) {
    // jshint ignore:line
    if (useAsync === false) throw new DOM.DOMException(DOM.NOT_SUPPORTED_ERR, "Zombie does not support synchronous XHR requests");
    ...
    if (/^(CONNECT|TRACE|TRACK)$/.test(method)) throw new DOM.DOMException(DOM.SECURITY_ERR, "Unsupported HTTP method");
    if (!/^(DELETE|GET|HEAD|OPTIONS|POST|PUT)$/.test(method)) throw new DOM.DOMException(DOM.SYNTAX_ERR, "Unsupported HTTP method");
    ...
    if (!/^https?:$/i.test(url.protocol)) throw new DOM.DOMException(DOM.NOT_SUPPORTED_ERR, "Only HTTP/S protocol supported");
    ...
    var request = { method: method, headers: headers, url: URL.format(url) };
  ...
  XMLHttpRequest.prototype.setRequestHeader = function setRequestHeader(header, value) {
    if (this.readyState !== XMLHttpRequest.OPENED) throw new DOM.DOMException(DOM.INVALID_STATE_ERR, "Invalid state");
    var request = this._pending;
  ...
  XMLHttpRequest.prototype.send = function send(data) {
    ...
    this._window._eventQueue.http(request.method, request.url, request, function (error, response) {
      ...
      if (request.aborted) {
        ...
        _this._error = new DOM.DOMException(DOM.ABORT_ERR, "Request aborted");
      ...
      if (error) {
        ...
        if (error.code === "ETIMEDOUT") {
          _this._error = new DOM.DOMException(DOM.TIMEOUT_ERR, "The request timed out");
          _this._fire("timeout", _this._error);
        } else {
          _this._error = new DOM.DOMException(DOM.NETWORK_ERR, error.message);
      ...
      if (_this._cors) {
        ...
        if (!(allowedOrigin === "*" || allowedOrigin === _this._cors)) {
          _this._error = new DOM.DOMException(DOM.SECURITY_ERR, "Cannot make request to different domain");
          _this._browser.errors.push(_this._error);
          _this._stateChanged(XMLHttpRequest.DONE);
          _this._fire("progress");
          _this._fire("error", _this._error);
  ...
  XMLHttpRequest.prototype.getAllResponseHeaders = function getAllResponseHeaders() {
    ...
    if (this._response) {
      ...
      return _core.Object.keys(this._response.headers).map(function (name) {
        return [name.toLowerCase(), _this._response.headers[name]];
      }).map(function (pair) {
        return pair.join(": ");
      }).join("\n");
    } else {
  ...
  XMLHttpRequest.prototype._stateChanged = function _stateChanged(newState) {
  ...
  XMLHttpRequest.prototype._fire = function _fire(eventName, error) {
  ...
  XMLHttpRequest.prototype.raise = function raise(type, message, data) {
  ...
  _createClass(XMLHttpRequest, {
    status: {
      get: function () {
        ...
        return this._response ? this._response.statusCode : this._error ? 0 : null;
      }
    },
    statusText: {
      get: function () {
    ...
    responseText: {
      get: function () {
        ...
        var hasBody = this._response && this.readyState >= XMLHttpRequest.LOADING;
        if (hasBody) {
